				...
				<div class="panel-body">
					...
					<c:if test="${not empty success}">
						<div class="alert alert-dismissible alert-success">
							...
							<h4>
								<strong>Parabéns! ${sessionScope.usuario.nome}</strong>
							</h4>
					...
					<c:if test="${empty success && not possuiTurma}">
						<div class="alert alert-dismissible alert-success">
							<button type="button" class="close" data-dismiss="alert">×</button>
							<h4>
								<strong>Seja Bem Vindo(a)! ${sessionScope.usuario.nome}</strong>
							</h4>
						</div>
					</c:if>
					...
					<c:if test="${possuiTurma}">
						<div class="alert alert-dismissible alert-info">
							...
							<strong>${nomeFormatado},</strong> agora, você pode realizar sua
							<a href="#" class="alert-link">presença diaria</a>, além de
							visualizar mais informaçãoes sobre sua frequencia. Participe,
							pois é muito importante para sua avaliação!!!
						</div>
					</c:if>

					<h4>Turmas</h4>
					<c:if test="${not empty turmasInicial}">
						<table id="table-turmas" class="table table-striped table-hover">
							...
							<tbody class="text-view-info">
								<c:forEach var="turma" items="${turmasInicial}">
									<tr>
										...
										<c:if test="${turma.statusTurma eq 'FECHADA'}">
											<td><span
												class="label ${turma.statusTurma eq 'ABERTA' ? 'label-success':'label-danger'}">${turma.statusTurma}</span></td>
										</c:if>
